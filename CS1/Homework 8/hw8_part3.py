...
if __name__ == '__main__':
    ...
    print()
    turn = 0
    while (len(berryField.bears) > 0 or len(berryField.bearQueue) > 0) or (len(berryField.bears) > 0 and berryField.berry_count() > 0):
        turn += 1
        ...
        print()
        if len(berryField.bears) > 0 and berryField.berry_count() > 0:
            print()
    print('Field has {} berries.'.format(berryField.berry_count()))
    print(str(berryField))
    print('Active Bears:')
