...
RecommendationSystem::RecommendationSystem()
{
	...
	// Makes 2D arrats
	userMovieRatingsMatrix = new int*[numUsers];
	for (int i = 0; i < numUsers; ++i)
	{
		userMovieRatingsMatrix[i] = new int[numMovies];
		for (int j = 0; j < numMovies; ++j)
		{
			userMovieRatingsMatrix[i][j] = 0;
		}
	}
	userShowRatingsMatrix = new int*[numUsers];
	for (int i = 0; i < numUsers; ++i)
	{
		userShowRatingsMatrix[i] = new int[numShows];
...
RecommendationSystem::~RecommendationSystem()
{
    ...
    delete[] userMovieRatingsMatrix;
	for (int i = 0; i < numUsers; ++i)
    {
        delete[] userShowRatingsMatrix[i];
    }
    delete[] userShowRatingsMatrix;
}

// Setters
void RecommendationSystem::setMovieRatings(const int& user, const int& movie, const int& rating)
{
	userMovieRatingsMatrix[user][movie] = rating;
}
void RecommendationSystem::setShowRatings(const int& user, const int& show, const int& rating)
{
	userShowRatingsMatrix[user][show] = rating;
}
